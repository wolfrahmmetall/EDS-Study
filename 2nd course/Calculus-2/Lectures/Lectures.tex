...
\definition \label{1.9} Функция $f$ интегрируема (по Риману) на замкнутом брусе $I$ ($f:I\rightarrow\R$), если $$\exists A\in\R: \forall \varepsilon > 0\ \exists \delta > 0: \forall(\T, \xi): \Delta_{\T} < \delta\: |\sigma(f, \T, \xi)| - A| < \varepsilon$$
Тогда 
...
$$
    \begin{tikzpicture}[scale=3]
        ...
        \foreach \j in {0.25, 0.5, ..., 2} {
            \fill (\i,\j) circle (1pt);  % Рисуем точку в координатах (\i, \j)
        }
    }
    
        % \fill[black] (0.25,0.25) circle (1pt);
        % \fill[black] (0.25,0.5) circle (1pt);
        % \fill[black] (0.25,0.75) circle (1pt);
        % \fill[black] (0.5,0.25) circle (1pt);
        % \fill[black] (0.5,0.5) circle (1pt);
        % \fill[black] (0.75,0.25) circle (1pt);
        % \fill[black] (1.75,1.75) circle (1pt);
    \end{tikzpicture}
$$
\end{minipage}
